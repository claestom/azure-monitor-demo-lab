...
| `setup-sre-agent.ps1` | Validates SRE Agent trial readiness and managed identity RBAC. Read-only unless `-GrantMissingRoles` is explicitly supplied. | `./scripts/setup-sre-agent.ps1 -SubscriptionId <sub> -ResourceGroup <rg> -AgentPrincipalId <object-id>` |
